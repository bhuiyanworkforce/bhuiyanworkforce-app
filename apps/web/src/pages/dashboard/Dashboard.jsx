...
export default function Dashboard() {
  ...
  // Both agent and assistant cannot see financial data
  const hideFinance = isAgent || isAssistant
  ...
  async function fetchAll() {
    ...
    try {
      ...
      const results = await Promise.allSettled([
        ...
        hideFinance ? Promise.resolve({ data: [], error: null }) : supabase.from('invoices').select('total, status, due_date'),
        ...
        hideFinance ? Promise.resolve({ data: [], error: null }) : supabase.from('cheques').select('amount, type, status').limit(500),
        hideFinance ? Promise.resolve({ data: [], error: null }) : supabase.from('expenses').select('amount').gte('date',
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center h-64 gap-4 px-4">
      ...
      <button onClick={fetchAll} className="bg-indigo-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm">Retry</button>
    </div>
  )

  // Quick action buttons — agent sees none, assistant sees candidate/passport/visa only
  const quickActions = isAgent ? [] : [
    ...
    ...(isAssistant ? [
      { label: 'Add Visa', icon: FileText, path: '/visa', color: 'from-amber-500 to-orange-600' },
    ] : [
      { label: 'New Invoice', icon: Wallet, path: '/accounts', color: 'from-pink-500 to-rose-600' },
    ]),
  ]

  // Stat cards — agent & assistant see no money
  const statCards = [
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Quick actions — hidden for agent */}
      {quickActions.length > 0 && (
        <div className={`grid gap-2 grid-cols-${quickActions.length}`}>
      ...
      {/* Overdue alert — owner/manager only */}
      {!hideFinance && stats.overdueCount > 0 && (
      ...
      {/* Expiring passports — everyone sees this */}
      ...
      {/* Passport pipeline — everyone sees this */}
      ...
      {stats.totalPassports === 0 && stats.totalCandidates === 0 && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 flex flex-col items-center text-center">
          ...
          <h3 className="text-slate-200 font-bold mb-2">Welcome to Bhuiyan Books</h3>
