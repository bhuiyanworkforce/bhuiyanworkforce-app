  ...
  async function fetchSubAgents() {
    ...
    try {
      const [
        ...
        supabase.from('invoices').select('sub_agent_id, total, status'),
        supabase.from('candidates').select('sub_agent_id').is('archived_at', null),
      ])
      if (e1) throw e1
      ...
      const agentsWithStats = subAgents.map(subAgent => {
        const agentInvoices = invoices.filter(i => i.sub_agent_id === subAgent.id)
        const paidTotal = agentInvoices
          .filter(i => i.status === 'paid')
          .reduce((s, i) => s + Number.parseFloat(i.total || 0), 0)
        const commission = (paidTotal * (Number.parseFloat(subAgent.commission_rate) || 0)) / 100
        const candidateCount = candidates.filter(c => c.sub_agent_id === subAgent.id).length
        return { ...subAgent, candidateCount, paidTotal, commission }
      })

      setSubAgents(agentsWithStats)
      setSummary({
        total: agentsWithStats.length,
        totalCommission: agentsWithStats.reduce((s, a) => s + a.commission, 0),
      })
    } catch (err) {
      console.error('[SubAgents] fetchSubAgents failed:', err)
      setFetchError(err.message || 'Failed to load sub agents')
      setSubAgents([])
    } finally {
      setLoading(false)
    }
  ...
  function renderSubAgentList() {
    ...
    if (fetchError) {
      return (
        <div className="bg-slate-900 border border-red-500/30 rounded-2xl p-6 flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-slate-300 text-sm font-semibold">Failed to load sub agents</p>
          <p className="text-slate-500 text-xs">{fetchError}</p>
          <button onClick={fetchSubAgents}
    ...
    if (subAgents.length === 0) {
      return (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 flex flex-col items-center text-center">
          ...
          <h3 className="text-slate-200 font-bold mb-2">No Sub Agents Yet</h3>
          <p className="text-slate-500 text-sm mb-4">Add your first sub agent to track commissions</p>
    ...
    return (
      <div className="flex flex-col gap-3">
        {subAgents.map((subAgent, i) => (
          <button key={subAgent.id} onClick={() => setSelected(subAgent)}
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${COLORS[i % COLORS.length]} flex items-center justify-center text-white font-extrabold text-lg flex-none`}>
                {subAgent.full_name?.[0]?.toUpperCase() || 'A'}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-slate-100 font-bold text-base truncate">{subAgent.full_name}</p>
                <p className="text-slate-500 text-xs">{subAgent.phone}</p>
              </div>
              <div className="flex items-center gap-1 bg-indigo-500/15 px-2.5 py-1 rounded-full">
                ...
                <span className="text-indigo-400 text-xs font-bold">{subAgent.commission_rate}%</span>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {[
                { label: 'Candidates', value: subAgent.candidateCount },
                { label: 'Revenue',    value: '৳' + (subAgent.paidTotal || 0).toLocaleString() },
                { label: 'Commission', value: '৳' + (subAgent.commission || 0).toLocaleString() },
              ].map(m => (
    ...
    <>
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-100">Sub Agents</h1>
            <p className="text-slate-500 text-sm">{subAgents.length} total</p>
          </div>
          <button onClick={() => setShowAdd(true)}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
            ...
            <p className="text-slate-500 text-xs mt-0.5">Total Sub Agents</p>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
      ...
      {showAdd && (
        <AddSubAgentModal onClose={() => setShowAdd(false)} onSaved={() => { setShowAdd(false); fetchSubAgents() }} />
      )}
      {selected && (
        <SubAgentDetail subAgent={selected} onClose={() => { setSelected(null); fetchSubAgents() }} />
      )}
    </>
  )
}
