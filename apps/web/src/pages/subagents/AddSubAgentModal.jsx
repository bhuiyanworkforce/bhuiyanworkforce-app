...
export default function AddSubAgentModal({ onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    const { error: err } = await supabase.from('sub_agents').insert({
    ...
    <div className="fixed inset-0 z-50 bg-black/70 flex items-end sm:items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-sm max-h-[92vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 sticky top-0 bg-slate-900">
          <h2 className="text-base font-bold text-slate-100">Add Sub Agent</h2>
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          {field('Full Name', 'full_name', 'text', 'Sub agent full name')}
          {field('Phone', 'phone', 'tel', '01XXXXXXXXX')}
          {field('Email', 'email', 'email', 'subagent@example.com')}
          {field('Company', 'company', 'text', 'Company name')}
          <div>
            <label htmlFor="subAgent-field-commission_rate" className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Commission Rate (%)</label>
            <input
              id="subAgent-field-commission_rate"
