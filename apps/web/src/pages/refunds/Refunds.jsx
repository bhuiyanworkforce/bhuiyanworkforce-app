  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#050D1A] overflow-y-auto">
      <div className="sticky top-0 z-10 bg-[#080F1E] border-b border-slate-800 px-4 py-3 flex items-center gap-3">
        ...
        <span className={`text-xs font-bold uppercase px-3 py-1 rounded-full ${STATUS_COLOR[refund.status] || 'bg-slate-700 text-slate-300'}`}>{refund.status}</span>
      </div>
      <div className="p-4 flex flex-col gap-4">
        {updateError && <p className="text-red-400 text-sm bg-red-500/10 px-4 py-3 rounded-xl">{updateError}</p>}
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <p className="text-slate-200 text-sm font-semibold capitalize">{refund.payment_method?.replaceAll('_', ' ') || '—'}</p>
            </div>
            <div>
        ...
        {refund.status === 'pending' && (
          <div className="flex flex-col gap-3">
            <button onClick={() => updateStatus('approved')} disabled={updating} className="w-full bg-gradient-to-r from-indigo-500 to-violet-600 text-white font-bold py-3.5 rounded-2xl disabled:opacity-50">✓ Approve Refund</button>
            <button onClick={() => updateStatus('paid')} disabled={updating} className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold py-3 rounded-2xl disabled:opacity-50">Mark as Paid</button>
          </div>
        )}
        {refund.status === 'approved' && (
          <button onClick={() => updateStatus('paid')} disabled={updating} className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold py-3.5 rounded-2xl disabled:opacity-50">✓ Mark as Paid</button>
        )}
        {refund.status === 'paid' && (
...
export default function Refunds() {
  ...
  async function fetchRefunds() {
    ...
    try {
      ...
      setError('Failed to load refunds. Tap refresh to try again.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowAdd(true)} className="flex items-center gap-2 bg-gradient-to-r from-red-500 to-rose-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-lg">
            <Plus size={16}/> Add
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        {['all', 'pending', 'approved', 'paid'].map(s => (
          <button key={s} onClick={() => setFilter(s)} className={`px-4 py-1.5 rounded-full text-xs font-bold capitalize transition-colors ${filter === s ? 'bg-indigo-500 text-white' : 'bg-slate-800 text-slate-400'}`}>{s}</button>
        ))}
      </div>
      ...
      {error ? (
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
          {filtered.length === 0 ? (
            ...
            <ul>
              {filtered.map((r, i) => (
                <li key={r.id} className={`${i < filtered.length - 1 ? 'border-b border-slate-800' : ''}`}>
                  <button type="button" onClick={() => setSelected(r)} className="w-full flex items-center gap-3 px-4 py-4 active:bg-slate-800 transition-colors text-left">
                    <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-col items-end gap-1.5 flex-none">
                      ...
                      <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${STATUS_COLOR[r.status] || 'bg-slate-700 text-slate-300'}`}>{r.status}</span>
                    </div>
