...
function buildInvoiceFilter(query, filter) {
  if (filter === 'overdue') {
    const today = new Date().toISOString().split('T')[0]
    return query.lt('due_date', today).not('status', 'in', '("paid","cancelled")')
  }
  return filter !== 'all' ? query.eq('status', filter) : query
}
...
async function fetchInvoicesBySearch(currentSearch, currentFilter, newOffset) {
  function base() {
    ...
    return buildInvoiceFilter(q, currentFilter)
  }
