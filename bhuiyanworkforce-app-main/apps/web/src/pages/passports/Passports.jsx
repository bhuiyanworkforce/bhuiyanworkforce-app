...
function mergeDedupeById(items) {
  const seen = new Set()
...
async function fetchPassportsBySearch(supabaseClient, { searchTerm, status, from, to, newOffset }) {
  ...
  const merged = mergeDedupeById([...(byPassportNo.data || []), ...(byCandidateName.data || [])])
...
function filterBtnClass(isActive) {
  return 'px-3 py-1 rounded-full text-xs font-bold ' + (isActive ? 'bg-indigo-500 text-white' : 'bg-slate-800 text-slate-400')
}
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {STATUSES.map(s => (
                  <button
                    ...
                    className={filterBtnClass(statusFilter === s) + ' capitalize'}
                  >
                    {s.replaceAll('_', ' ')}
                  </button>
                ))}
